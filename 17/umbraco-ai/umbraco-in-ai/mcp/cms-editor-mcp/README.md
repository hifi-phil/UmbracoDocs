...
The Editor [MCP Server](https://app.gitbook.com/s/NnUDSAShVcdETvjguL9v/model-context-protocol#mcp-servers) gives content editors and managers a natural, conversational way to work with Umbraco. It allows you to use AI assistants to manage content, media, translations, and more, without needing developer tools or technical setup.
...
[MCP clients](https://app.gitbook.com/s/NnUDSAShVcdETvjguL9v/model-context-protocol#mcp-clients) are implemented inside compatible [host applications](https://app.gitbook.com/s/NnUDSAShVcdETvjguL9v/model-context-protocol#host-applications) such as Claude Desktop, Cursor, or Windsurf. These clients connect to the server. When you interact with your chat-based environment, the client communicates with the MCP Server using the Model Context Protocol (MCP).
