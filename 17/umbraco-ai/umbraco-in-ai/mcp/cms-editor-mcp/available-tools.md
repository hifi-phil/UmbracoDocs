...
- `report-orphan-pages` — Find pages with no inbound content references from other pages. These orphan pages may be unreachable or forgotten.
...
- `report-unused-media` — Find media items not referenced by any content page. Unused files waste storage and clutter the media library.
...
- `report-content-references` — Find which pages reference a given content page or media item. Useful before deleting or restructuring content.
